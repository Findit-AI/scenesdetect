...
pub(super) unsafe fn gradient_anisotropy(mag: &[i32], dir: &[u8], w: usize, h: usize) -> f32 {
  ...
  for bin_val in 0..4 {
    ...
    // internally (no saturating-i64 intrinsic on AVX2), but the final
    // scalar combine clamps to u64::MAX so the histogram never wraps
    // to a smaller value than the scalar reference would.
    let bin_sum = lanes.iter().fold(0u64, |a, &x| a.saturating_add(x as u64));
    hist[bin_val] = hist[bin_val].saturating_add(bin_sum);
  }

  super::gradient_anisotropy_score_from_hist(&hist)
}

/// AVX2 mean-absolute-difference reducer.
///
/// 32 bytes per iteration: `_mm256_sad_epu8` sums 32 u8 absolute
/// differences into 4 u16 partial-sums in i32 lanes 0/4/8/12 (per
/// 64-bit half), then the result is added into an `i64×4`
/// accumulator. At the end we horizontal-reduce to a scalar `u64`,
/// add the scalar tail, and divide by `n`.
///
/// # Safety
///
/// Caller must ensure AVX2 is available.
...
pub(super) unsafe fn mean_abs_diff(a: &[u8], b: &[u8], n: usize) -> f64 {
  ...
  while i < whole {
    ...
    let sad = unsafe { _mm256_sad_epu8(va, vb) };
    acc = unsafe { _mm256_add_epi64(acc, sad) };
    i += LANES;
  }

  // Horizontal reduce u64×4 → u64.
  let mut lanes = [0i64; 4];
  unsafe { _mm256_storeu_si256(lanes.as_mut_ptr() as *mut __m256i, acc) };
  let mut sum: u64 = lanes.iter().fold(0u64, |a, &x| a.wrapping_add(x as u64));
...
/// AVX2 Sobel 3×3. Processes 16 interior pixels per iteration —
/// double the SSSE3/SSE4.1 backend's 8-lane width.
///
/// Per chunk:
/// - Load 8 `u8×16` neighborhoods and widen to `i16×16` in
///   `__m256i` via `_mm256_cvtepu8_epi16`.
/// - Compute `gx`, `gy` as `i16×16`, then `mag_i16 = |gx| + |gy|`.
/// - Sign-extend `i16×16` → two `i32×8` via `_mm256_cvtepi16_epi32`
///   (per 128-bit half), and store as two 32-byte `__m256i` writes
///   into the magnitude plane.
/// - Direction stays scalar: transmute `gx`/`gy` to `[i16; 16]` and
///   run the same 4-bin classifier as the SSSE3 backend.
///
/// # Safety
///
/// Caller must ensure AVX2 is available.
...
pub(super) unsafe fn sobel(input: &[u8], mag: &mut [i32], dir: &mut [u8], w: usize, h: usize) {
  ...
  for y in 1..h.saturating_sub(1) {
    ...
    while x + LANES < w {
      macro_rules! ld {
        ($row:expr, $o:expr) => {{
          let v = unsafe { _mm_loadu_si128($row.as_ptr().add($o) as *const __m128i) };
      ...
      let gx = unsafe {
        let pos = _mm256_add_epi16(_mm256_add_epi16(pr, _mm256_slli_epi16::<1>(cr)), nr);
        let neg = _mm256_add_epi16(_mm256_add_epi16(pl, _mm256_slli_epi16::<1>(cl)), nl);
        _mm256_sub_epi16(pos, neg)
      };
      let gy = unsafe {
        let pos = _mm256_add_epi16(_mm256_add_epi16(nl, _mm256_slli_epi16::<1>(nm)), nr);
        let neg = _mm256_add_epi16(_mm256_add_epi16(pl, _mm256_slli_epi16::<1>(pm)), pr);
        _mm256_sub_epi16(pos, neg)
      };

      let mag_i16 = unsafe { _mm256_add_epi16(_mm256_abs_epi16(gx), _mm256_abs_epi16(gy)) };

      // i16×16 → two i32×8 via cvtepi16_epi32 on each 128-bit half.
      // Note: gx, gy are in [-1020, 1020], so |gx|+|gy| ≤ 2040 stays
      // positive as i16 — sign-extend matches zero-extend here.
      let lo_128 = unsafe { _mm256_castsi256_si128(mag_i16) };
      let hi_128 = unsafe { _mm256_extracti128_si256::<1>(mag_i16) };
      let lo = unsafe { _mm256_cvtepi16_epi32(lo_128) };
      let hi = unsafe { _mm256_cvtepi16_epi32(hi_128) };
      unsafe {
        _mm256_storeu_si256(mag.as_mut_ptr().add(off + x) as *mut __m256i, lo);
        _mm256_storeu_si256(mag.as_mut_ptr().add(off + x + 8) as *mut __m256i, hi);
      }

      let gx_arr: [i16; 16] = unsafe { core::mem::transmute(gx) };
      let gy_arr: [i16; 16] = unsafe { core::mem::transmute(gy) };
      for j in 0..LANES {
...
/// AVX2 BGR → BT.601 luma: `Y = (77·R + 150·G + 29·B) >> 8`.
///
/// 16 pixels per iteration. The 24-byte BGR deinterleave reuses
/// the same 9-pshufb pattern as [`bgr_to_hsv_planes`] (128-bit
/// shuffles within 256-bit lanes; cross-lane AVX2 permutes aren't
/// worth the complexity at this width). After deinterleave we
/// widen each channel `u8×16 → i16×16` with `_mm256_cvtepu8_epi16`,
/// do the coefficient MAC in 256-bit, shift right by 8, and pack
/// to `u8×16` via [`pack_avx2`].
///
/// # Safety
///
/// Caller must ensure AVX2 (which implies SSSE3) is available.
...
pub(super) unsafe fn bgr_to_luma(
  ...
  let k_b = unsafe { _mm256_set1_epi16(29) };
  let k_g = unsafe { _mm256_set1_epi16(150) };
  let k_r = unsafe { _mm256_set1_epi16(77) };
  ...
  for y in 0..h {
    ...
    while x < whole {
      ...
      // Coefficient MAC. Max per lane: 29·255 + 150·255 + 77·255 = 65280
      // (sum of coeffs is 256), which fits in u16 / i16 unsigned-positive.
      let acc = unsafe {
        _mm256_add_epi16(
          _mm256_add_epi16(_mm256_mullo_epi16(b16, k_b), _mm256_mullo_epi16(g16, k_g)),
          _mm256_mullo_epi16(r16, k_r),
        )
      };
      // Shift right by 8 → Y in [0, 255] as i16×16.
      let y_i16 = unsafe { _mm256_srli_epi16::<8>(acc) };

      // Pack i16×16 to u8×16 within 256-bit lanes; permute lanes back into
      // sequential order. Same pattern as `pack_avx2` but only one input
      // vector — duplicate the lanes via permute.
      let packed = unsafe { _mm256_packus_epi16(y_i16, y_i16) };
      // After packus, low half holds y[0..8] then y[8..16] is in lane 1 (i.e.
      // the second 128-bit chunk). Bits [0..63] = y[0..8], bits [128..191] =
      // y[8..16]. Permute4x64 with control 0b1101_1000 (= [0,2,1,3])
      // re-interleaves: lane0=lo[0..8], lane1=hi[0..8], so the low 128 bits
      // become the contiguous 16 bytes we want.
      let perm = unsafe { _mm256_permute4x64_epi64::<0b1101_1000>(packed) };
      let out_vec = unsafe { _mm256_castsi256_si128(perm) };
      unsafe {
        _mm_storeu_si128(out.as_mut_ptr().add(dst_off + x) as *mut __m128i, out_vec);
      }
...
/// AVX2 clipping-pixel count.
///
/// `max(R, G, B)` is byte-order-agnostic, so this kernel takes
/// no `ChannelOrder` parameter. The 9-pshufb 24→3×16 deinterleave
/// is the same as [`clipping_count`]'s SSSE3/SSE4.1 form — there
/// is no real AVX2 win on this kernel because the inner reduction
/// is dominated by `_mm_max_epu8` + `subs_epu8` + `_mm_sad_epu8`,
/// which AVX2 doesn't speed up at 128-bit width. The function
/// exists so the dispatcher ladder is uniform; the body is the
/// SSSE3 algorithm under `target_feature = "avx2"`.
///
/// # Safety
///
/// Caller must ensure AVX2 (which implies SSSE3) is available.
...
/// AVX2 Tenengrad sharpness: `mean(gx² + gy²)` over a 3×3 Sobel.
///
/// 16 interior pixels per chunk — double the SSSE3/SSE4.1 width.
/// Widen each of 8 neighborhood positions from `u8×16` to `i16×16`
/// via `_mm256_cvtepu8_epi16`, build `gx` / `gy` as `i16×16`,
/// then `_mm256_madd_epi16` on interleaved `(gx, gy)` pairs gives
/// per-pixel `gx² + gy²` as `i32×8` (peak `2·1020² = 2 080 800`
/// fits in `i32`). Each `i32×8` half is widened to `i64×4` via
/// `_mm256_cvtepi32_epi64` (sign-extend; non-negative) and added
...
pub(super) unsafe fn tenengrad(luma: &[u8], w: usize, h: usize, s: usize) -> f32 {
  ...
  for y in 1..h - 1 {
    ...
    while x < x_vec_end {
      ...
      let two_l = unsafe { _mm256_slli_epi16::<1>(l) };
      let two_r = unsafe { _mm256_slli_epi16::<1>(r) };
      let pos_x = unsafe { _mm256_add_epi16(_mm256_add_epi16(tr, two_r), br) };
      let neg_x = unsafe { _mm256_add_epi16(_mm256_add_epi16(tl, two_l), bl) };
      let gx = unsafe { _mm256_sub_epi16(pos_x, neg_x) };

      // gy = -tl - 2t - tr + bl + 2b + br
      let two_t = unsafe { _mm256_slli_epi16::<1>(t) };
      let two_b = unsafe { _mm256_slli_epi16::<1>(b) };
      let pos_y = unsafe { _mm256_add_epi16(_mm256_add_epi16(bl, two_b), br) };
      let neg_y = unsafe { _mm256_add_epi16(_mm256_add_epi16(tl, two_t), tr) };
      let gy = unsafe { _mm256_sub_epi16(pos_y, neg_y) };

      // Interleave gx and gy at i16 granularity, then madd against
      // self gives per-pair sum of squares (i32×8 result per side).
      // Note: AVX2 unpack operates within 128-bit lanes, so the two
      // outputs correspond to halves [0..8] and [8..16] of each
      // 128-bit chunk, not contiguous halves of the 16-wide vector.
      // That doesn't matter here because we sum all 16 squared
      // contributions into a single scalar at the end.
      let lo_pair = unsafe { _mm256_unpacklo_epi16(gx, gy) };
      let hi_pair = unsafe { _mm256_unpackhi_epi16(gx, gy) };
      let sq_lo = unsafe { _mm256_madd_epi16(lo_pair, lo_pair) };
      let sq_hi = unsafe { _mm256_madd_epi16(hi_pair, hi_pair) };

      // Widen i32×8 → i64×4 × 2 (squares are non-negative).
      let lo_128 = unsafe { _mm256_castsi256_si128(sq_lo) };
      let hi_128 = unsafe { _mm256_extracti128_si256::<1>(sq_lo) };
      acc = unsafe { _mm256_add_epi64(acc, _mm256_cvtepi32_epi64(lo_128)) };
      acc = unsafe { _mm256_add_epi64(acc, _mm256_cvtepi32_epi64(hi_128)) };

      let lo_128 = unsafe { _mm256_castsi256_si128(sq_hi) };
      let hi_128 = unsafe { _mm256_extracti128_si256::<1>(sq_hi) };
      acc = unsafe { _mm256_add_epi64(acc, _mm256_cvtepi32_epi64(lo_128)) };
      acc = unsafe { _mm256_add_epi64(acc, _mm256_cvtepi32_epi64(hi_128)) };
...
/// AVX2 single-pass `(mean, variance)` on a u8 plane.
///
/// 32-byte chunks: `_mm256_sad_epu8(v, zero)` produces four u64
/// partial-sums of u8 lanes (one per 64-bit half-lane within each
/// 128-bit half) for the sum-of-x stream. For the sum-of-x²
/// stream, widen `u8×32` to two `u16×16` halves and apply
/// `_mm256_madd_epi16(v, v)` for pair-sums of squares (`i32×8`
/// per half, lane peak `2·65 025 = 130 050`). Each `i32×8` is
/// then widened to `i64×4` via `_mm256_cvtepi32_epi64` and added
/// into the running accumulator. Same `E[X²] - E[X]²` derivation
/// as SSSE3.
///
/// # Safety
///
...
pub(super) unsafe fn plane_mean_variance(plane: &[u8], w: usize, h: usize, s: usize) -> (f32, f32) {
  ...
  let zero = unsafe { _mm256_setzero_si256() };
  let mut sum_acc = unsafe { _mm256_setzero_si256() }; // u64×4
  let mut sq_acc = unsafe { _mm256_setzero_si256() }; // u64×4
  ...
  for y in 0..h {
    ...
    let mut x = 0;
    while x < whole {
      let v = unsafe { _mm256_loadu_si256(plane.as_ptr().add(row_base + x) as *const __m256i) };

      // sum_x — PSADBW against zero gives four u64 partials in
      // 64-bit half-lanes 0/2 of each 128-bit half.
      let sad = unsafe { _mm256_sad_epu8(v, zero) };
      sum_acc = unsafe { _mm256_add_epi64(sum_acc, sad) };

      // sum_x²: widen u8×32 → two u16×16 halves, then madd against
      // self yields per-pair sums of squares (peak 2·255² = 130 050,
      // fits in i32 without overflow). Each i32×8 half is widened
      // to i64×4 (non-negative; sign-extend matches zero-extend).
      let v_lo = unsafe { _mm256_cvtepu8_epi16(_mm256_castsi256_si128(v)) };
      let v_hi = unsafe { _mm256_cvtepu8_epi16(_mm256_extracti128_si256::<1>(v)) };
      let pair_lo = unsafe { _mm256_madd_epi16(v_lo, v_lo) };
      let pair_hi = unsafe { _mm256_madd_epi16(v_hi, v_hi) };

      let pair_lo_lo = unsafe { _mm256_castsi256_si128(pair_lo) };
      let pair_lo_hi = unsafe { _mm256_extracti128_si256::<1>(pair_lo) };
      sq_acc = unsafe { _mm256_add_epi64(sq_acc, _mm256_cvtepi32_epi64(pair_lo_lo)) };
      sq_acc = unsafe { _mm256_add_epi64(sq_acc, _mm256_cvtepi32_epi64(pair_lo_hi)) };

      let pair_hi_lo = unsafe { _mm256_castsi256_si128(pair_hi) };
      let pair_hi_hi = unsafe { _mm256_extracti128_si256::<1>(pair_hi) };
      sq_acc = unsafe { _mm256_add_epi64(sq_acc, _mm256_cvtepi32_epi64(pair_hi_lo)) };
      sq_acc = unsafe { _mm256_add_epi64(sq_acc, _mm256_cvtepi32_epi64(pair_hi_hi)) };
  ...
  // Reduce u64×4 → u64.
  let mut sum_lanes = [0i64; 4];
  let mut sq_lanes = [0i64; 4];
  unsafe {
    _mm256_storeu_si256(sum_lanes.as_mut_ptr() as *mut __m256i, sum_acc);
    _mm256_storeu_si256(sq_lanes.as_mut_ptr() as *mut __m256i, sq_acc);
  }
  let sum_vec: u64 = sum_lanes
    .iter()
    .fold(0u64, |a, &x| a.wrapping_add(x as u64));
  let sq_vec: u64 = sq_lanes.iter().fold(0u64, |a, &x| a.wrapping_add(x as u64));
